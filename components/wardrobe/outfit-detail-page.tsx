...
export function OutfitDetailPage({ look }: { look: Look }) {
  ...
  const originals: Array<{ src?: string; label: string }> = look.source === "LOCAL_UPLOAD" ? [{ label: `本地上传图片 · ${look.date}` }] : [look.person ? { src: look.person, label: "人物" } : null, look.garment ? { src: look.garment, label: "服装" } : null].filter((item): item is { src: string; label: string } => Boolean(item));
  ...
  return <div className="outfit-detail-page"><div className="outfit-detail-layout"><div className="outfit-detail-stage"><StickerStage interactive containBackground onSettingsChange={save} sticker={sticker} settings={settings}><BackgroundStepper value={settings} onChange={saveNow} /></StickerStage></div><aside className="outfit-detail-side"><h1>穿搭详情</h1><div className="outfit-detail-actions"><button className="detail-side-btn" onClick={() => router.push("/wardrobe")}><ArrowLeft size={17} />返回</button><button className="detail-side-btn danger" onClick={() => void deleteLook()}><Trash2 size={17} />删除</button><BackgroundPicker value={settings} onChange={saveNow} onUpload={(event) => { const file = event.target.files?.[0]; if (!file) return; void fileToDataUrl(file).then((backgroundImage) => save({ ...settings, backgroundKey: "custom", backgroundImage })).catch(() => setMessage("背景上传失败")); event.target.value = ""; }} /><button className="detail-side-btn" onClick={() => void downloadComposite().catch(() => setMessage("下载失败"))}><Download size={17} />下载图片</button><button className="detail-side-btn primary" onClick={() => setDateOpen(true)}><CalendarDays size={17} />添加到日历</button></div>{originals.length > 0 && <div className={`outfit-originals ${look.source === "LOCAL_UPLOAD" ? "local-upload-originals" : ""}`} aria-label="原始照片" data-date={look.date}>{originals.map((item) => <div className="outfit-original-item" key={item.label}>{item.src && <img src={item.src} alt={item.label} />}<span>{item.label}</span></div>)}</div>}{message && <p className="result-message">{message}</p>}</aside></div>{dateOpen && <DatePickerModal close={() => setDateOpen(false)} confirm={(date) => void addDate(date)} />}</div>;
}
