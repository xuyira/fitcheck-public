...
export function WardrobeGrid() {
  ...
  return <>{error && <p className="form-error">{error}</p>}<div className="look-grid"><button className="add-tile" onClick={() => setAddOpen(true)}><span><Plus size={22} /></span><b>添加新穿搭</b></button>{wardrobeLooks.map((look) => <article className="look-card" key={look.id}><div className="look-image" onClick={() => router.push(`/wardrobe/${look.id}`)}><StickerStage className="wardrobe-preview" containBackground trimSticker={false} sticker={look.sticker ?? look.image} settings={settingsFor(look)} /><button className="card-delete" aria-label="删除" onClick={(event) => { event.stopPropagation(); void deleteLook(look.id); }}><Trash2 size={16} /></button></div></article>)}</div>{addOpen && <AddChoiceModal close={() => setAddOpen(false)} startTry={() => router.push("/try")} uploadImage={(event) => void uploadImage(event)} />}{dateOpen && selected && <DatePickerModal close={() => setDateOpen(false)} confirm={(date) => void addToCalendar(selected, date)} />}{uploadError && <UploadErrorModal message={uploadError} confirm={() => setUploadError("")} />}</>;
}
