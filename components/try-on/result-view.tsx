...
export function ResultView({ resultImage, stickerImage, person, garment, initialBackground, regenerate }: ResultViewProps) {
  ...
  const ensureSaved = async (saveToWardrobe = true) => {
    ...
    const { outfit } = await apiFetch<{ outfit: Look }>("/api/outfits", { method: "POST", body: JSON.stringify({ finalImage: resultImage, stickerImage, source: "AI_TRY_ON", saveToWardrobe, personImage: person, garmentImage: garment, backgroundImage: settings.backgroundImage, backgroundKey: settings.backgroundKey, stickerScale: settings.stickerScale }) });
  ...
  const requestSave = (target: "wardrobe" | "calendar", authenticated = false) => {
    ...
    void ensureSaved(true).then(() => setMessage("已保存到衣橱")).catch((e) => setMessage(e instanceof Error ? e.message : "保存失败"));
  };
  const addToCalendar = async (date: string) => {
    try { const outfit = await ensureSaved(false); await apiFetch("/api/calendar", { method: "POST", body: JSON.stringify({ outfitId: String(outfit.id), date, backgroundImage: settings.backgroundImage, backgroundKey: settings.backgroundKey, stickerScale: settings.stickerScale }) }); setDateOpen(false); setMessage("已添加到日历"); }
