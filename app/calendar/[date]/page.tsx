...
export default async function CalendarDatePage({ params }: { params: Promise<{ date: string }> }) {
  ...
  const entry = await db.calendarEntry.findFirst({ where: { userId: user.id, date }, include: { outfit: { select: { id: true, source: true, finalImage: true, stickerImage: true, personImage: true, garmentImage: true, backgroundImage: true, backgroundKey: true, stickerScale: true, stickerOffsetX: true, stickerOffsetY: true, createdAt: true } } } });
