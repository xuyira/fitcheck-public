...
export async function DELETE(_: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const deleted = await db.calendarEntry.deleteMany({ where: { id, userId: user.id } });
    if (!deleted.count) return NextResponse.json({ error: "日历记录不存在" }, { status: 404 });
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await db.calendarEntry.findFirst({ where: { id, userId: user.id } });
    if (!existing) return NextResponse.json({ error: "日历记录不存在" }, { status: 404 });
    ...
    return NextResponse.json({ entry: serializeCalendarEntry(entry) });
  } catch (error) { return apiError(error); }
}
